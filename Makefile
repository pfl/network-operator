...
# intel/intel-network-operator-bundle:$VERSION and intel/intel-network-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= intel/intel-network-operator
...
manifests: controller-gen ## Generate WebhookConfiguration, ClusterRole and CustomResourceDefinition objects.
	$(CONTROLLER_GEN) rbac:roleName=manager-role paths="./internal/..." output:dir=config/operator/rbac
	$(CONTROLLER_GEN) webhook paths="./api/..." output:webhook:artifacts:config=config/operator/webhook
	$(CONTROLLER_GEN) crd:crdVersions=v1 paths="./api/..." output:crd:artifacts:config=config/operator/crd/bases
...
test: manifests generate fmt vet envtest ## Run tests.
	...
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" go test $$(go list ./... | grep -v -e /test -e /pkg/lldp -e internal/version) -coverprofile cover.out
...
build-installer: manifests generate kustomize ## Generate a consolidated YAML with CRDs and deployment.
	...
	$(KUSTOMIZE) build config/operator/default > dist/install.yaml
